...
- Critical or High findings, or any Dependency Check finding with CVSS >= 7, must fail the pipeline unless the team creates a written exception and intentionally adjusts the gate.
...
| `ajv` v6 | Medium | CVE-2025-69873 | Transitive via webpack/schema-utils dev tooling | A blanket override from AJV 6 to AJV 8 can break webpack/schema-utils consumers. Finding is limited to build/dev tooling in this repo. | Keep as a documented Medium exception for MVP; resolve by upgrading Nest CLI/webpack toolchain in a separate tooling PR. |
